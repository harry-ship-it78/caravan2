...
export default function Pile({
  ...
  isDropAllowedCard,
  onCanDropCard,
  // tap-to-place payload from App (selected card in hand)
  selectedPayload
}) {
  ...
  // Tap-to-place valid-target highlighting when a card is selected in hand
  const canDropHere = useMemo(() => {
    if (!selectedPayload) return { container: false, perCard: [] };
    const perCard = visibleOrder.map((_, idx) => !!onCanDropCard?.(idx, selectedPayload));
    const container = !!onCanDropContainer?.(selectedPayload);
    return { container, perCard };
  }, [selectedPayload, visibleOrder, onCanDropCard, onCanDropContainer]);

  // Container drag-n-drop (always preventDefault when droppable so drop can fire)
  ...
  // Tap-to-place on container (numbers / aces)
  const handleContainerClick = useCallback(() => {
    if (!selectedPayload || !isDropAllowedContainer) return;
    if (onCanDropContainer && !onCanDropContainer(selectedPayload)) return;
    onDropContainer?.(JSON.stringify(selectedPayload));
  }, [selectedPayload, isDropAllowedContainer, onCanDropContainer, onDropContainer]);
  ...
  // Tap-to-place on a specific card (faces only)
  const handleCardClick = useCallback((idx) => {
    if (!selectedPayload || !isDropAllowedCard) return;
    if (onCanDropCard && !onCanDropCard(idx, selectedPayload)) return;
    onDropCard?.(idx, JSON.stringify(selectedPayload));
  }, [selectedPayload, isDropAllowedCard, onCanDropCard, onDropCard]);
  ...
  return (
    ...
    >
      ...
      <div
        className={`pile-cards stacked ${canDropHere.container ? 'drop-target' : ''}`}
        style={{ height: positions.height }}
        onPointerLeave={clearHover}
        onClick={handleContainerClick}
        role="button"
        tabIndex={0}
        aria-label={`Place number/Ace on ${owner} pile ${pileIndex + 1}`}
      >
        ...
        {visibleOrder.map((card, idx) => {
          ...
          if (canDropHere.perCard[idx]) classes.push('card-target'); // valid target highlight for tap
          ...
          return (
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label={`Target ${card.rank}${glyph}${card.removed ? ' (removed)' : ''}`}
            >
              <div className="corner tl">
                <div className="rank">{card.rank}</div>
                <div className="suit">{glyph}</div>
              </div>
              <div className="center small" aria-hidden="true">{glyph}</div>
